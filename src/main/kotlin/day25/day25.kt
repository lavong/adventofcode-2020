...
fun main() {
    val input = AdventOfCode.file("day25/input").lines().filterNot { it.isEmpty() }.map { it.toLong() }

    val subjectNumber = input[0]
    val secret = guessLoopCount(input[1])

    println("solution: ${transform(1, secret, subjectNumber)}")
}

fun transform(value: Long, loopCount: Long, subjectNumber: Long): Long {
    var result = value
    (0 until loopCount).forEach { _ ->
        result = (result * subjectNumber) % 20201227
    }
    return result
}

fun guessLoopCount(publicKey: Long): Long {
    var value = 1L
    var loopCount = 0L
    while (true) {
        if (value == publicKey) {
