...
enum class TerrainType(val char: Char) {
    ...
    companion object {
        fun forChar(c: Char) = values().find { c == it.char }
            ?: throw IllegalArgumentException("unexpected terrain type: $c")
    }
}
...
fun main() {
    ...
    val terrain = mutableListOf<Array<TerrainType>>()
        .apply { input.forEach { add(it.terrainTypes()) } }
        .toTypedArray()
...
fun traverse(terrain: Array<Array<TerrainType>>, dx: Int, dy: Int): Long {
    ...
    (dy until terrain.size step dy).onEach { y ->
        ...
        //println("tree on $x,$y: ${terrain[y][x] == TerrainType.TREE}")
    }
