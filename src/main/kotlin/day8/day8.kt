...
fun main() {
    ...
    val input = AdventOfCode.file("day8/input").lines().filter { it.isNotBlank() }

    val instructions = input
        .map { it.split(" ") }
        .map { Instruction(Operation.valueOf(it[0]), it[1].toInt()) }
...
fun execute(instructions: Map<Int, Instruction>): Int {
    ...
    while (!instructionsExecuted.contains(ptr)) {
        ...
        when (instruction.op) {
            Operation.nop -> ptr++
            Operation.acc -> ptr++.also { acc += instruction.arg }
